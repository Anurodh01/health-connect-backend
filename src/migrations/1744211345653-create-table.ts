...
export class CreateTable1744211345653 implements MigrationInterface {
    name = 'CreateTable1744211345653'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`doctor_profiles\` (\`id\` int NOT NULL AUTO_INCREMENT, \`speciality\` enum ('physician', 'dentist', 'ent', 'Cardiologist', 'Neurologist', 'Gynecologist', 'Oncologist') NOT NULL DEFAULT 'physician', \`degree\` enum ('mbbs', 'md', 'do', 'bds', 'bhms', 'bams', 'dnb', 'pharmd', 'ms', 'msc', 'phd', 'physio', 'other') NOT NULL DEFAULT 'other', \`clinicAddress\` text NOT NULL, \`location\` text NOT NULL, \`availability\` text NOT NULL, \`verified\` enum ('0', '1') NOT NULL DEFAULT '0', \`createdAt\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`ALTER TABLE \`reviews\` ADD CONSTRAINT \`FK_426d6805968f17a40bf7426e1fa\` FOREIGN KEY (\`profileId\`) REFERENCES \`doctor_profiles\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
